...
file and rebuilds `normalized_keys` automatically. The file is generated by a
local-only builder script, so persistent additions should be made through it
rather than by hand — otherwise regenerating the mapping can remove them.
