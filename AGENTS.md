...
For the full design rationale, anti-bot history, and edge cases, see @docs/touhou_tagger_context.md — this file only holds what must be in context every session.
...
- **Most tags are skip-if-present**: if the file already has a non-empty value, leave it untouched (`date`, `year`, `catalognumber`, credits, and TouhouDB-sourced `artist`/`artistsort`).
...
- **Romanization resolver priority**: TouhouDB official name > THBWiki Staff-section mapping > original name (with the CJK guard suppressing a CJK fallback from ever landing in a sort field).
- Field-by-field tag IDs (ID3/M4A/Vorbis) and the full write-policy tables are in @docs/touhou_tagger_context.md — check there before adding a new tag.
...
- Any change to THBWiki/English-wiki fetch logic should be cross-checked against the anti-bot findings in @docs/touhou_tagger_context.md before assuming a simpler approach will work; most "simpler" approaches were already tried and documented as failing.
