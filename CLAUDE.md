...
- For GitHub-related work, if a command fails in the restricted sandbox and completing the request may require running it outside the sandbox, first ask the user for explicit permission to retry it outside the sandbox. Do not retry outside the sandbox without that permission.
